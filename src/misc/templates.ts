...
{{#readFunctions}}
const {{identifier}}QueryBtn = document.getElementById("{{identifier}}_query_btn");
const {{identifier}}Outputs = document.getElementById("{{identifier}}_outputs");
{{identifier}}QueryBtn.addEventListener("click", async () => {
  {{identifier}}QueryBtn.innerText = "Querying...";
  {{identifier}}QueryBtn.disabled = "disabled";
  {{identifier}}Outputs.innerText = "";
 {{#hasInputs}}
  {{#inputs}}
  ...
  if ({{#inputs}}{{identifier}}_{{inputName}}_input.toString().length === 0 || {{/inputs}} false) {
    {{identifier}}Outputs.innerText = "Invalid input(s)";
...
{{#writeFunctions}}
const {{identifier}}ExecuteBtn = document.getElementById("{{identifier}}_execute_btn");
const {{identifier}}Outputs = document.getElementById("{{identifier}}_outputs");
{{identifier}}ExecuteBtn.addEventListener("click", async () => {
  {{identifier}}ExecuteBtn.innerText = "Executing...";
  {{identifier}}ExecuteBtn.disabled = "disabled";
  {{identifier}}Outputs.innerText = "";
  {{#hasInputs}}
  {{#inputs}}
  const {{identifier}}_{{inputName}}_input = document.getElementById("{{identifier}}_{{inputName}}_input").value;
  ...
  {{/hasInputs}}
    {{identifier}}ExecuteBtn.innerText = "Waiting Tx confirm...";
    {{identifier}}Outputs.innerHTML = 'txHash: <a href="{{&blockchainExplorer}}' + tx.hash + '" target="_blank">' + tx.hash + '</a>';
    await tx.wait();
...
const {{identifier}}OnBtn = document.getElementById("{{identifier}}_on_btn");
{{identifier}}OnBtn.addEventListener("click", async () => {
  console.log("~~~ start listen '{{name}}' event inputs=[{{#inputs}}{{inputName}}, {{/inputs}}]");

  {{identifier}}OnBtn.innerText = "Listening";
  {{identifier}}OnBtn.disabled = "disabled";
...
const {{identifier}}OffBtn = document.getElementById("{{identifier}}_off_btn");
{{identifier}}OffBtn.addEventListener("click", async () => {
  console.log("--- stop listen '{{name}}' event");

  {{identifier}}OnBtn.innerText = "Start Listen New Event";
  {{identifier}}OnBtn.disabled = "";
...
<div id="{{identifier}}_read_function_div" style="border: 2px solid orange; border-radius: 8px; padding: 16px; margin: 8px">
  ...
  {{#hasInputs}}
    ...
    <input id="{{identifier}}_{{inputName}}_input" type="text" /><br />
    {{/inputs}}
  {{/hasInputs}}
  </div>
